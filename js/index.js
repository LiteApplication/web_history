...
function loadButtons(el) { // fonction d'initalisation des boutons
    ...
    for (i in elements) { // En math, ça donne i --> n elements --> U elements[i] = U(n) = un élément de la page en particulier
        if (elements[i].hasAttribute && elements[i].hasAttribute('target_page')) { //Si l'element à des attributs et si il a un attribut target_page
...
function toggleButton(el) { // définit fonction qui charge les pages et maintient les boutons activés (en bleu)
    ...
    for (i in elements) {
        if (elements[i].hasAttribute && elements[i].hasAttribute('target_page')) {
    ...
    setTimeout(function () { // Attend 200 ms avant d'executer la fonction (le temps que pageContent disparaisse)
        setContentText(`${host}/web_history/${el.getAttribute("target_page")}`); // Probablement la ligne la plus complexe, mais facile si on la décompose:
        //  Partie 1 : `${host}/web_history/${el.getAttribute("target_page")}` ce code construit l'url à charger :
        //      host : https://claj-ndc.github.io
        //      /web_history/
        //      el.getAttribute("target_page") : le contenu de target_page du bouton sur lequel on vient de cliquer (ex : pages/informations.html)
